...
public:
  ...
  void recut(int win_count)
  {
    if (octo_state != 1)
    {
      ...
      // **修改点 1**: 使用 clear() 而不是 swap() 来清空 vector，减少内存重分配
      sig_orig.clear();
      sig_tran.clear();
      sig_orig.shrink_to_fit();
      sig_tran.shrink_to_fit();
...
public:
  ...
  void damping_iter(vector<IMUST> &x_stats, VOX_HESS &voxhess)
  {
    ...
    for (int i = 0; i < (int)voxhess.plvec_voxels.size(); i++)
    {
      for (int j = 0; j < (int)voxhess.plvec_voxels[i]->size(); j++)
