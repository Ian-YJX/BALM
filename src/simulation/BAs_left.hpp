...
public:
  ...
  void left_jacobian_point(const vector<IMUST> &xs, int beg, int end, Eigen::MatrixXd &Rcov)
  {
    ...
    for (int a = beg; a < end; a++)
    {
      ...
      Eigen::Vector4d SpTul = Sp.transpose() * u[l];
      PLM(4)
      T_FC(win_size, Eigen::Matrix4d::Zero());
      ...
      for (int j = 0; j < win_size; j++)
        if (Ns[j] != 0)
        {
          ...
          Eigen::MatrixXd Ls(6 * win_size, 9);
          Ls.setZero();
          for (int p = 0; p < win_size; p++)
            if (Ns[p] != 0)
            {
    ...
    for (int a = beg; a < end; a++)
    {
      ...
      for (int i = 0; i < win_size; i++)
        if (sig_orig[i].N != 0)
        {
          ...
          Eigen::Matrix<double, 3, 6> PRuk_rho;
          PRuk_rho.setZero();
...
public:
  ...
  OCTO_TREE_NODE()
  {
    value_vector.setZero();
    max_dis = 0;
    octo_state = 0;
    push_state = 0;
    vec_orig.resize(win_size + fix_size);
    vec_tran.resize(win_size + fix_size);
    sig_orig.resize(win_size + fix_size);
    sig_tran.resize(win_size + fix_size);
    for (int i = 0; i < 8; i++)
      leaves[i] = nullptr;
...
public:
  ...
  OCTO_TREE_ROOT()
  {
    ...
    for (int i = 0; i < win_size + fix_size; i++)
      each_num[i] = 0;
  }
...
{
  // ROS_INFO("Entering cut_voxel function. Processing frame %d with %lu points.", fnum, pl_feat.size());

  float loc_xyz[3];
  int max_each_num = 0; // 监测最大的 each_num[fnum]

  for (size_t i = 0; i < pl_feat.points.size(); i++)
  {
    ...
    {
      OCTO_TREE_ROOT *ot = iter->second;
      int &num = ot->each_num[fnum];
      ...
      {
        ROS_WARN("Voxel (%ld, %ld, %ld) reached max size %d! Skipping push_back.", position.x, position.y, position.z, num);
      }
    ...
    {
      // ROS_INFO("Voxel not found. Creating new voxel at (%ld, %ld, %ld)", position.x, position.y, position.z);
    ...
    // 🟢 处理 1000 个点后打印一次日志，防止日志过多
    // if (i % 1000 == 0)
    // {
    //   ROS_INFO("Processed %zu/%lu points in frame %d. Max each_num = %d", i, pl_feat.points.size(), fnum, max_each_num);
    // }
  }

  // ROS_INFO("Exiting cut_voxel function for frame %d. Max each_num = %d", fnum, max_each_num);
}
...
//   if (feat_map.size() == 0)
//   {
//     feat_map.reserve(100000000); // 预估 500 万个 key
//   }
//   float loc_xyz[3];
