...
int count_csv_rows(const std::string& filename) {
  ...
  if(count%4 != 0)
  {
    cerr << "ERROR: The number of rows in the file is not a multiple of 4." << endl;
  }
  return count/4;
}

ros::Publisher pub_curr, pub_full, pub_path;
...
int main(int argc, char **argv)
{
  ...
  fstream inFile(file_path + "Log/update_pose.csv", ios::in);
  int pose_size = count_csv_rows(file_path + "Log/update_pose.csv");
  ...
  for (int i = 0; i < pose_size; i++)
  {
    ...
    for (int j = 0; j < 4; j++)
    {
      ...
      {
        cerr << "ERROR: Failed to read line from" << file_path + "Log/update_pose.csv " << "at pose " << i << " row " << j << endl;
        return -1;
      }
      ...
      while (getline(ss, str, ','))
      {
        ...
        {
          cerr << "ERROR: Invalid double conversion at pose " << i << " row " << j << ": " << str << endl;
          return -1;
        }
      }
    }
    ...
    if (nums.size() < 16)
    {
      cerr << "ERROR: Not enough data to form 4x4 matrix at pose " << i << ". Got " << nums.size() << " values." << endl;
      return -1;
    }
  ...
  for (int iterCount = 0; iterCount < 1 && n.ok(); iterCount++)
  {
    ...
    {
      ostringstream oss;
      oss << file_path << "PCDs/" << setw(6) << setfill('0') << m + 1 << ".pcd";
      ...
      ROS_INFO("Loading Progress: %d/%d", m + 1, pose_size);
      ...
      Eigen::MatrixXd Rcov(6 * win_size, 6 * win_size);
      Rcov.setZero();
      BALM2 opt_lsv;
